...
# Based on work by iRobot, Zach Dodds, Peter Mawhorter, C.A. Berry as 
# updated for SIGCSE 3/9/07

# v2.5 Made work with Python 2.5 to 2.7.2
# set to work in millimeters instead of centimeters
# Gary Anderson: ganderson at seattle academy dot org
...
SOFT_RESET = chr(7)    # undocumented soft reset
START = chr(128)    # Starts and puts in passive mode
BAUD = chr(129)     # + 1 byte to set baud rate
CONTROL = chr(130)  # deprecated for Create
SAFE = chr(131)     # Put in safe mode (does not fall down stairs, etc.
FULL = chr(132)     # Put in full mode without safe protection
POWER = chr(133)    
...
# the above command is called "Cover and Dock" on the Creates
DRIVEDIRECT = chr(145)       # Create only + 4 bytes for left and right wheel
...
#======================== Starting up and Shutting Down================    
    def __init__(self, PORT='/dev/ttyUSB0', startingMode=SAFE_MODE, sim_mode = False):
        """ the constructor which tries to opern the
...
#========================= Moving Around ================================================    
    ...
    def go( self, mmPerSec=0, degPerSec=0 ):
        """ go(mmPerSec, degPerSec) sets the robot's linear velocity to
               mmPerSec millimeters per second and its angular velocity to
               degPerSec degrees per second
            go() is equivalent to go(0,0)
        """
        degPerSec = float(degPerSec)
        mmPerSec = float(mmPerSec)
        if mmPerSec == 0:
            ...
            radPerSec = math.radians(float(degPerSec))
            velMmSec = mmPerSec
    ...
    def driveDirect( self, leftMmSec=0, rightMmSec=0 ):
        """ Go(mmpsec, degpsec) sets the robot's velocity to
               mmpsec millimeters per second
               degpsec degrees per second
            Go() is equivalent to go(0,0)
        """
        """ sends velocities of each wheel independently
               left_mm_sec:  left  wheel velocity in mm/sec
               right_mm_sec: right wheel velocity in mm/sec
        """
